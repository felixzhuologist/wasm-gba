    ...
    fn process_instruction(&self, regs: &mut Registers) {
        ...
        let (result, carry_out) = match self.opcode {
            ...
            Op::ADC => {
                ...
                let (r2, c2) = r1.overflowing_add(regs.cpsr.c as u32);
                (r2, c1 || c2)
            },
            Op::SBC => {
                ...
                let (result, add_overflow) = r2.overflowing_add(regs.cpsr.c as u32);
